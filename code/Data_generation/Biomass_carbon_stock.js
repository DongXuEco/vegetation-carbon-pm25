/*
 * Fit stacked biomass models and predict annual carbon stock.
 * Use matching predictor bands and units for training and prediction.
 * Output units follow the sample target field.
 */
...
// Point samples may represent biomass, soil organic carbon (SOC), or soil
// inorganic carbon (SIC). Use one target variable per run and set targetProperty
// to its numeric field name. Predictions retain the units of that field.
var samplesAsset = '';
...
if (runAnnualPrediction) {
    for (var year = predictionStartYear; year <= predictionEndYear; year++) {
        if (year !== referenceYear && !predictionAssets[year]) {
...
if (runValidation) {
    ...
    exportImage(predictionSD.addBands(uncertainty), 'Carbon_uncertainty_' + referenceYear);
    if (exportResults) {
        Export.table.toDrive({collection: foldMetrics, folder: exportFolder,
            description: 'Carbon_cv_metrics', fileFormat: 'CSV'});
        Export.table.toDrive({collection: foldPredictions, folder: exportFolder,
            description: 'Carbon_cv_predictions', fileFormat: 'CSV',
...
if (exportResults) {
    Export.table.toDrive({collection: testMetrics, folder: exportFolder,
        description: 'Carbon_test_metrics', fileFormat: 'CSV'});
    Export.table.toDrive({collection: testPredictions, folder: exportFolder,
        description: 'Carbon_test_predictions', fileFormat: 'CSV',
...
Map.addLayer(predicted_RF_final_final, band_P, 'Carbon stock ' + referenceYear, false);
exportImage(predicted_RF_final_final, 'Carbon_stock_' + referenceYear);

// Annual prediction uses the same predictor schema and fitted models.
if (runAnnualPrediction) {
    for (var i = predictionStartYear; i <= predictionEndYear; i++) {
        if (i === referenceYear) continue;
        ...
        exportImage(predicted_RF_final_final_1, 'Carbon_stock_' + i);
    }
}
